import React, { Component } from "react";
import { Button, Collapse, Form } from "react-bootstrap";

export default class EmployeeFilter extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: false,
    };
  }

  onTitleChange = (e) => {
    const { value } = e.target;
    this.props.filterByTitle(value);
  };

  onDepartmentChange = (e) => {
    const { value } = e.target;
    this.props.filterByDepartment(value);
  };

  onEmployeeTypeChange = (e) => {
    const { value } = e.target;
    this.props.filterByEmployeeType(value);
  };

  onStatusChange = (e) => {
    const { value } = e.target;
    this.props.filterByStatus(value);
  };

  onRetirementChange = (e) => {
    const { value } = e.target;
    this.props.filterByUpcoming(value);
  };

  clearFilters = () => {
    this.setState({
      selectedTitle: "",
      selectedDepartment: "",
      selectedEmployeeType: "",
      selectedStatus: "",
      selectedRetirement: "",
    });
    this.props.filterByTitle("");
    this.props.filterByDepartment("");
    this.props.filterByEmployeeType("");
    this.props.filterByStatus("");
    this.props.filterByUpcoming("");
  };

  render() {
    const { departments, titles, employeeTypes } = this.props;
    const { selectedTitle, selectedDepartment, selectedEmployeeType, selectedStatus, selectedRetirement } = this.state;

    return (
      <div>
        <Button
          variant="light"
          onClick={() => this.setState({ open: !this.state.open })}
          aria-controls="filter-collapse"
          aria-expanded={this.state.open}
          className="mb-3 w-25"
        >
          Filter
          <span className="bi bi-filter"></span>
        </Button>
        <Collapse in={this.state.open}>
          <div id="filter-collapse" className="mb-3">
            <Form className="d-flex gap-3">
              <Form.Select
                aria-label="Employee Title"
                value={selectedTitle}
                onChange={this.onTitleChange}
              >
                <option>Select Title</option>
                {titles.map((title) => (
                  <option key={title} value={title}>
                    {title}
                  </option>
                ))}
              </Form.Select>
              <Form.Select
                aria-label="Employee Type"
                value={selectedEmployeeType}
                onChange={this.onEmployeeTypeChange}
              >
                <option>Select Employee Type</option>
                {employeeTypes.map((employeeType) => (
                  <option key={employeeType} value={employeeType}>
                    {employeeType}
                  </option>
                ))}
              </Form.Select>
              <Form.Select
                aria-label="Employee Department"
                value={selectedDepartment}
                onChange={this.onDepartmentChange}
              >
                <option>Select Department</option>
                {departments.map((department) => (
                  <option key={department} value={department}>
                    {department}
                  </option>
                ))}
              </Form.Select>
              <Form.Select
                aria-label="Employee Status"
                value={selectedStatus}
                onChange={this.onStatusChange}
              >
                <option>Select Status</option>
                <option value="Working">Working</option>
                <option value="Retired">Retired</option>
              </Form.Select>
              <Form.Select
                aria-label="Employee Retirement"
                value={selectedRetirement}
                onChange={this.onRetirementChange}
              >
                <option>Select Upcoming</option>
                <option value="All">All</option>
                <option value="Retirement">Retirement</option>
              </Form.Select>
              <Button
                variant="outline-primary"
                className="w-50 fw-semibold"
                onClick={this.clearFilters}
              >
                Clear Filters
              </Button>
            </Form>
          </div>
        </Collapse>
      </div>
    );
  }
}